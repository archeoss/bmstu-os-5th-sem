\begin{titlepage}
	...
	\begin{center}
		\fontsize{18pt}{0.6cm}\selectfont 
			Тема:
			\textit{<<Обработчик прерывания системного таймера и пересчет динамических приоритетов>>}
	...
	\begin{table}[h!]
		...
		\begin{signstabular}[0.55]{p{7.25cm} >{\centering\arraybackslash}p{4cm} >{\centering\arraybackslash}p{4cm}}
				Студент \uline{~~ИУ7-55Б~~} & \uline{\mbox{\hspace*{4cm}}} & \uline{\hfill \textbf{С. К. Романов} \hfill} \\
		...
		\begin{signstabular}[0.55]{p{7.25cm} >{\centering\arraybackslash}p{4cm} >{\centering\arraybackslash}p{4cm}}
			Преподаватель & \uline{\mbox{\hspace*{4cm}}} & \uline{\hfill \textbf{Н. Ю. Рязанова} \hfill} \\
